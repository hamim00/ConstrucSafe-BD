    ...
    st.markdown(
        ...
        <div class="flagged-item">
            <div style="display:flex; align-items:center; margin-bottom:0.5rem; gap:0.4rem;">
                ...
                <span style="margin-left:auto; font-size:0.75rem; color:#5D4037; opacity:0.7;">
                    {t_severity(sev, lang)}
                </span>
            </div>
            <p style="margin:0.25rem 0; font-size:0.88rem; color:#5D4037;">{desc}</p>
            <div style="font-size:0.82rem; color:#BF360C; margin-top:0.4rem;">
                <strong>{why_label}:</strong> {flag_reason}
            </div>
            {"<div style='font-size:0.78rem; color:#795548; margin-top:0.25rem; font-style:italic;'>" + assumption_note + "</div>" if assumption_note else ""}
        </div>
        ''',
